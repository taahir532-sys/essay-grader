...
st.set_page_config(page_title="TEFLMate v5.4 Clean", page_icon="📝", layout="centered")
st.markdown("""<style>.stButton>button {background:#111;color:white;border-radius:10px;height:48px;font-weight:bold;width:100%;} a[data-testid="stLinkButton"]{background:#111;color:white;border-radius:10px;height:48px;font-weight:bold;width:100%; text-align:center;}</style>""", unsafe_allow_html=True)
...
def init_paystack(email, amount_kobo, plan_code):
    try:
        ...
        # NO callback_url - prevents not_found redirect
...
# Auto-unlock if?reference= present (from Paystack if it redirects, or manual link)
query_params = st.query_params
if "reference" in query_params:
    ref = query_params["reference"]
    if unlock_from_verify(verify_paystack(ref)):
...
st.title("📝 TEFLMate v5.4 - Batch Grader")
...
with st.sidebar:
    ...
    if g['code'] == "ZAR":
        ...
        if "ONCE10" in st.session_state.pay_links:
            st.link_button(f"👉 Pay R{g['once']} Now (Click Here)", st.session_state.pay_links["ONCE10"], use_container_width=True)
        if "WEEK49" in st.session_state.pay_links:
            st.link_button(f"👉 Pay R{g['weekly']} Now (Click Here)", st.session_state.pay_links["WEEK49"], use_container_width=True)
        ...
        if "MONTH99" in st.session_state.pay_links:
            st.link_button(f"👉 Pay R{g['monthly']} Now (Click Here)", st.session_state.pay_links["MONTH99"], use_container_width=True)
        if "YEAR799" in st.session_state.pay_links:
            st.link_button(f"👉 Pay R{g['yearly']} Now (Click Here)", st.session_state.pay_links["YEAR799"], use_container_width=True)
        ...
        if st.session_state.last_ref:
            if st.button("✅ I PAID - Click to Unlock PRO", type="primary", use_container_width=True):
                ...
                if not unlock_from_verify(v):
                    st.warning("Payment not confirmed yet. Wait 5 sec after Paystack says Success, then click again.")
        st.caption("After paying on Paystack, return here and click Unlock. Auto-unlocks, no redirect to error page.")
        st.divider()
    ...
    if st.button("Unlock Code"):
        now = datetime.now()
        if code in ["TEACH10","WEEK49","MONTH99","YEAR799","TEFL2026"]:
            if code == "TEACH10": st.session_state.uses = max(0, st.session_state.uses - 10)
            elif code == "WEEK49": st.session_state.pro_expiry = now + timedelta(days=7)
            elif code == "MONTH99": st.session_state.pro_expiry = now + timedelta(days=30)
            elif code == "YEAR799": st.session_state.pro_expiry = now + timedelta(days=365)
            elif code == "TEFL2026": st.session_state.pro_expiry = now + timedelta(days=30)
            st.success("Unlocked!")
            st.rerun()
        else:
            st.error("Bad code")
...
with tab1:
    ...
    if st.button("GRADE ESSAY ->"):
        if not is_pro() and st.session_state.uses >= 3:
            st.error("Free limit reached - Pay in sidebar")
            st.stop()
...
with tab2:
    ...
    sample_df = pd.DataFrame({"student_name": ["Student 1", "Student 2"], "essay": ["I go to market yesterday.", "My friend is Thandi."]})
    csv_template = sample_df.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Download CSV Template", csv_template, file_name="TEFLMate_Template.csv", mime="text/csv", key="template_btn")
    level_b = st.selectbox("Target Level for batch:", ["A1","A2","B1","B2","C1","C2"], key="batch_level")
    uploaded = st.file_uploader("Upload CSV or TXT", type=["csv","txt"], key="batch_file")
    if st.button("GRADE BATCH 50 ->"):
        if not is_pro():
            st.error(f"Batch 50 needs Monthly PRO")
            st.stop()
        if not uploaded:
...
with tab4:
    ...
    st.table(pd.DataFrame([{"Level": "A1", "Class": "Grade 1-3", "Use": "ABET"}, {"Level": "A2", "Class": "Grade 4-6", "Use": "Primary"}, {"Level": "B1", "Class": "Grade 7-9", "Use": "High"}, {"Level": "B2", "Class": "Matric", "Use": "Matric"}, {"Level": "C1", "Class": "University", "Use": "Uni"}, {"Level": "C2", "Class": "Mastery", "Use": "Teachers"}]))

st.divider()
st.caption("TEFLMate v5.4 • Durban, SA • Built by Mr Taahir Mahomed")
