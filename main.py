...
if has_token:
    ...
    new_p = st.text_input("New Password (6+ chars)", type="password", key="rec_new_final_1920")
    conf_p = st.text_input("Confirm", type="password", key="rec_conf_final_1920")
...
def login_screen():
    ...
    with t1:
        with st.form("login_form_final_1920"):
        ...
        with st.expander("🔓 Forgot Password?"):
            fp_email = st.text_input("Email to reset:", key="fp_email_1920")
    ...
    with t2:
        with st.form("signup_form_final_1920"):
            email2 = st.text_input("New Email", key="s_email_1920"); password2 = st.text_input("New Password", type="password", key="s_pass_1920"); school = st.text_input("School Name", value="My School")
...
def extract_score_cefr(text):
    try:
        score_match = re.search(r'(\d+)\s*/\s*10', text); score = int(score_match.group(1)) if score_match else 5
...
def detect_ai_risk(essay_text):
    ...
    words = text.split()
    if len(words) < 3: return "Too Short"
    avg = sum(len(w) for w in words)/len(words) if words else 0
    if avg>6.0 and len([w for w in words if len(w)>10])>5: return "⚠️ Possible AI"
    return "✅ Human"
...
def grade_with_groq(essay_text, level):
    ...
    prompt = f"You are Cambridge TEFL examiner for {level}. {std_inst}. {lang_inst}. {rubric} Grade this essay even if short, give fair score minimum 3/10 if at least 5 words: {essay_text}. ASCII only. Structure: 1.CEFR+{std} 2.Score/10 3.AI Check 4.Summary 5.2 Strengths 6.Table Mistake|Correction|Why 7.Corrected version 8.Parent Summary"
...
with st.sidebar:
    ...
    with st.expander("🔐 Change Password"):
        new_pass = st.text_input("New Password", type="password", key="new_pass_1920")
        confirm_pass = st.text_input("Confirm New Password", type="password", key="confirm_pass_1920")
    ...
    with st.expander("🎟️ Enter Code / Promo"):
        promo = st.text_input("Promo Code:", value="", placeholder="Enter code", key="promo_code_empty_1920")
        if st.button("✅ Redeem Code", use_container_width=True, key="redeem_btn_1920"):
    ...
    email = st.text_input("Email for receipt:", value=YOUR_EMAIL, key="pay_email_1920")
    ...
    try:
        ...
        if rows.data:
            ...
            if not is_pro():
                ...
                st.download_button("📄 Principal PDF - All Students (PRO)", create_principal_pdf(rows_for_pdf, level_pdf, avg, "My School"), file_name="Portfolio_Principal.pdf", use_container_width=True, key="portfolio_pdf_1920")
            st.divider()
            ...
            sel = st.selectbox("Select essay to delete:", list(delete_options.keys()), key="del_select_1920")
            c_del1, c_del2 = st.columns(2)
            with c_del1:
                if st.button("🗑️ Delete Selected", use_container_width=True, key="del_btn_1920"):
                    try: supabase.table("essays").delete().eq("id", delete_options[sel]).execute(); st.success("Deleted!"); st.rerun()
                    except Exception as e: st.error(f"Delete failed: {e}")
            with c_del2:
                if st.button("⚠️ Clear ALL", use_container_width=True, key="clear_all_1920"):
...
with tab1:
    essay = st.text_area("Paste Essay:", height=150, placeholder="Paste student essay..."); s_name = st.text_input("Student Name:", value="Student", key="s_name_single_1920"); level = st.selectbox("Target Level:", ["A1","A2","B1","B2","C1","C2"], key="single_level_1920")
    if st.button("🚀 GRADE & SAVE TO PORTFOLIO ->", type="primary", use_container_width=True):
        ...
        with st.spinner("Grading & saving..."):
            try:
                ...
                st.success(f"Saved: {score}/10 {cefr}"); st.markdown(f"**AI Check:** {ai_flag}"); st.markdown(result_text)
...
with tab3:
    ...
    level_p=st.selectbox("Target Level:",["A1","A2","B1","B2","C1","C2"],key="photo_level_1920")
    camera_pic=st.camera_input("Take photo"); upload_img=st.file_uploader("Upload Image",type=["jpg","jpeg","png"],key="img_up_1920"); upload_pdf=st.file_uploader("Upload PDF",type=["pdf"],key="pdf_up_1920")
    image_bytes=None
    ...
    if upload_pdf:
        extracted=extract_text_from_pdf(upload_pdf.getvalue()); st.text_area("Text from PDF:",value=extracted,height=120, key="pdf_text_1920")
    ...
    if image_bytes and st.button("📸 READ, GRADE & SAVE -> Portfolio (1 Click)", type="primary", use_container_width=True, key="read_grade_save_1920"):
        with st.spinner("Reading handwriting + Grading..."):
            extracted=extract_text_from_image(image_bytes); st.session_state.last_ocr=extracted
            try:
                result_text=grade_with_groq(extracted,level_p); score,cefr,_=extract_score_cefr(result_text); ai_flag=detect_ai_risk(extracted); save_essay_db("Photo Student",extracted,level_p,score,cefr,result_text); st.session_state.last_photo_result=result_text
                if not is_pro(): st.session_state.uses+=1
                st.success(f"✅ OCR + Graded + Saved: {score}/10 {cefr} | {ai_flag}"); st.text_area("OCR Text:", value=extracted, height=100, key="ocr_after_1920"); st.markdown(result_text)
                st.download_button("📄 Parent Report PDF", create_branded_pdf(extracted, result_text, level_p, "Photo Student"), file_name=f"Photo_Report_{level_p}.pdf", use_container_width=True, key="photo_pdf_1920")
            except Exception as e: st.error(f"Error: {e}"); st.text_area("OCR Text (fix & grade manually):", value=extracted, height=120, key="ocr_error_1920")
...
with tab2:
    ...
    sample_df = pd.DataFrame({
        ...
        "essay":[
            "Yesterday I went to the market with my mother. We bought vegetables and fruits. I liked the market because it was busy and colorful. I want to go again next week with my family.",
            "My best friend is Thandi. She is very kind and smart. We play together every day after school. She helps me with homework and we share lunch. I am lucky to have her as friend.",
            "Last week I broke my leg when I was playing football at school. I went to hospital and doctor put plaster. Now I stay at home and my friends visit me. I miss playing football."
        ]
    })
    b,_ = df_to_excel_bytes_safe(sample_df,"Essays"); st.download_button("📥 Download Template (50 students)", b, file_name="Template_50.xlsx", use_container_width=True, key="template_50_1920")
    school_name = st.text_input("School Name:", value="My School", key="school_name_1920"); level_b = st.selectbox("Target Level:", ["A1","A2","B1","B2","C1","C2"], key="batch_level_1920")
    uploaded = st.file_uploader("Upload Excel/CSV (max 50)", type=["csv","xlsx","txt"], key="batch_file_1920")
    if st.button("🚀 GRADE BATCH 50 -> Save to Portfolio", type="primary", use_container_width=True, key="batch_btn_50_1920"):
    ...
    if st.session_state.batch_results:
        ...
        if is_pro():
            st.download_button("📄 Principal PDF (PRO)", create_principal_pdf(excel_rows, level_b, avg_score, school_name), file_name=f"Principal_{school_name}.pdf", use_container_width=True, key="principal_pdf_1920")
...
with tab6:
    ...
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("🇧🇷 Brazil - Portuguese + CEFR", use_container_width=True, key="set_br_1920"): st.session_state.feedback_lang="Portuguese"; st.session_state.grading_standard="CEFR"; st.success("✅ Portuguese + CEFR - Brazil ready"); st.balloons()
        if st.button("🇲🇽 Mexico/Spain - Spanish + CEFR", use_container_width=True, key="set_es_1920"): st.session_state.feedback_lang="Spanish"; st.session_state.grading_standard="CEFR"; st.success("✅ Spanish + CEFR ready")
    with c2:
        if st.button("🇸🇦 MENA - Arabic + IELTS", use_container_width=True, key="set_ar_1920"): st.session_state.feedback_lang="Arabic"; st.session_state.grading_standard="IELTS"; st.success("✅ Arabic + IELTS - MENA ready"); st.balloons()
        if st.button("🇺🇸 USA/UK - English + TOEFL", use_container_width=True, key="set_us_1920"): st.session_state.feedback_lang="English"; st.session_state.grading_standard="TOEFL"; st.success("✅ English + TOEFL ready")
    with c3:
        if st.button("🇮🇳 India - Hindi + IELTS", use_container_width=True, key="set_in_1920"): st.session_state.feedback_lang="Hindi"; st.session_state.grading_standard="IELTS"; st.success("✅ Hindi + IELTS - India ready"); st.balloons()
        if st.button("🇿🇦 SA - English + CEFR", use_container_width=True, key="set_sa_1920"): st.session_state.feedback_lang="English"; st.session_state.grading_standard="CEFR"; st.success("✅ English + CEFR - SA ready")
