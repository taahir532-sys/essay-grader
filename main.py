...
.tefl-card {
    background:white; border:1px solid #eee; border-radius:16px; padding:16px;
...
window.addEventListener('beforeinstallprompt', (e) => {
  ...
  btn.style = 'position:fixed;bottom:20px;right:20px;background:#111;color:white;padding:14px 20px;border-radius:12px;font-weight:800;z-index:9999;border:none;box-shadow:0 4px 20px rgba(0,0,0,0.3)';
...
if "parent" in q_parent:
    try:
        ...
        if row.data:
            ...
            st.info("Generated by TEFLMate - Ask teacher for full portfolio link")
        else:
            st.error("Report not found or link expired")
        st.stop()
...
def login_screen():
    ...
    with col1:
        ...
        <div class="landing-hero">
            ...
            <ul>
                ...
                <li>✅ Parent reports in home language</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.success("On phone: Tap Share → Add to Home Screen → Works like Play Store app!")
    st.divider()
    ...
    with t2:
        ...
        if ref_code_from_url:
            st.success(f"🎁 Referral code {ref_code_from_url} applied - you both get bonus!")
        with st.form("signup_form_final_1967"):
            ...
            if st.form_submit_button("Create Account", use_container_width=True):
                try:
                    ...
                    if ref_code_from_url:
                        insert_data["referred_by"] = ref_code_from_url
        ...
        try:
            ...
            if ref_count >= 3:
                if st.button("🎉 Claim 1 Month Free (3 invites)", type="primary", use_container_width=True):
                    ...
                    st.session_state.pro_expiry = new_exp
                    st.session_state.active_plan = "REFERRAL-3"
                    st.success("✅ 1 Month Free Unlocked!")
                    st.balloons()
            else:
        ...
        if rows.data:
            ...
            if sel_parent:
                ...
                if eid:
                    ...
                    st.code(share_url)
                    st.caption("Copy & send to parent on WhatsApp - they see report without login")
            if not is_pro():
...
with tab3:
    ...
    if image_bytes and st.button("📸 READ, GRADE & SAVE -> Portfolio (1 Click)", type="primary", use_container_width=True, key="read_grade_save_1967"):
        with st.spinner("Reading handwriting + Grading + Saving..."):
            ...
            try:
                ...
                st.text_area("OCR Text:", value=extracted, height=100, key="ocr_after_1967"); st.markdown(result_text)
                st.download_button("📄 Parent Report PDF", create_branded_pdf(extracted, result_text, level_p, "Photo Student"), file_name=f"Photo_Report_{level_p}.pdf", use_container_width=True, key="photo_pdf_1967")
...
with tab6:
    ...
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("🇧🇷 Brazil - Portuguese + CEFR", use_container_width=True, key="set_br_1967"): st.session_state.feedback_lang="Portuguese"; st.session_state.grading_standard="CEFR"; st.success("✅ Portuguese + CEFR - Brazil ready"); st.balloons()
    ...
    with c2:
        if st.button("🇸🇦 MENA - Arabic + IELTS", use_container_width=True, key="set_ar_1967"): st.session_state.feedback_lang="Arabic"; st.session_state.grading_standard="IELTS"; st.success("✅ Arabic + IELTS - MENA ready"); st.balloons()
