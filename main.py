...
if has_token:
    ...
    new_p = st.text_input("New Password (6+ chars)", type="password", key="rec_new_1943")
    conf_p = st.text_input("Confirm", type="password", key="rec_conf_1943")
...
def login_screen():
    ...
    with t2:
        with st.form("signup_form_1943"):
            email2 = st.text_input("New Email", key="s_email_1943"); password2 = st.text_input("New Password", type="password", key="s_pass_1943"); school = st.text_input("School Name", value="My School")
...
def unlock_paystack(v):
    ...
    if amt == 1000 or plan == "ONCE10": st.session_state.uses -= 10; active="ONCE10"; save_pro_to_db(None, active, 10)
    elif amt == 4900 or plan == "WEEK49": pro_exp = now + timedelta(days=7); active="WEEK49"; save_pro_to_db(pro_exp, active, 0)
    elif amt == 9900 or plan == "MONTH99": pro_exp = now + timedelta(days=30); active="MONTH99"; save_pro_to_db(pro_exp, active, 0)
    elif amt == 79900 or plan == "YEAR799": pro_exp = now + timedelta(days=365); active="YEAR799"; save_pro_to_db(pro_exp, active, 0)
    else: return False
...
def grade_with_groq(essay_text, level):
    ...
    if std=="IELTS": std_inst="Also give IELTS Band"
    elif std=="TOEFL": std_inst="Also give TOEFL Score"
    elif std=="US Grade": std_inst="Also give US Grade A-F"
    else: std_inst="Give CEFR A1-C2"
    lang_inst = f"Feedback in {lang}" if lang!="English" else "Feedback in English"
    if level in ["A1","A2"]:
        level_inst = f"Target {level} - BE GENEROUS. 30+ words basic communication = 6/10 minimum. Good effort = 7-8/10. Only below 5/10 if <15 words. ALWAYS integer X/10, deterministic."
    else:
        level_inst = f"Target {level} - standard. ALWAYS integer X/10"
    prompt = f"You are kind Cambridge examiner for {level}. {level_inst}. {std_inst}. {lang_inst}. {rubric} Grade: {essay_text}. ASCII only. Score integer X/10. Be consistent same input same score. Structure: 1.CEFR 2.Score/10 integer 3.AI Check 4.Summary 5.Strengths 6.Table Mistake|Correction|Why 7.Corrected 8.Parent"
    res = client.chat.completions.create(
...
def save_essay_db(student_name, essay_text, level, score, cefr, feedback):
    try:
        if not st.session_state.teacher_id:
            ...
            return False
        resp = supabase.table("essays").insert({"teacher_id": st.session_state.teacher_id, "student_name": student_name, "essay_text": essay_text, "level": level, "score": score, "cefr": cefr, "feedback": feedback}).execute()
        return True
    except Exception as e:
...
with st.sidebar:
    ...
    email = st.text_input("Email for receipt:", value=YOUR_EMAIL, key="pay_email_1943")
    ...
    if st.session_state.pay_links:
        st.link_button(f"Once {g['symbol']}{g['once']}", st.session_state.pay_links.get("ONCE10","#"), use_container_width=True)
        ...
        if st.button("✅ Check Payment", type="primary", use_container_width=True):
            if verify_all_refs(): st.rerun()
    st.divider()
    st.session_state.feedback_lang = st.selectbox("Parent Lang:", ["English","Spanish","Portuguese","French","Arabic","Hindi","Chinese"], index=["English","Spanish","Portuguese","French","Arabic","Hindi","Chinese"].index(st.session_state.feedback_lang))
...
tab4, tab1, tab3, tab2 = st.tabs(["📚 Portfolio","✍️ Grade Essay","📸 Photo/PDF","⚡ Batch 50 PRO"])
    ...
    try:
        ...
        if rows.data:
            ...
            if is_pro():
                ...
                st.download_button("📄 Principal PDF", create_principal_pdf(rows_for_pdf, level_pdf, avg, "My School"), file_name="Principal.pdf", use_container_width=True, key="portfolio_pdf_1943")
            st.divider()
            delete_options = {f"{r['student_name']} | {r['score']}/10 | {str(r['created_at'])[:16]} | {r['id'][:6]}": r['id'] for r in rows.data}
            sel = st.selectbox("Select to delete:", list(delete_options.keys()), key="del_select_1943")
            if st.button("🗑️ Delete Selected", use_container_width=True, key="del_btn_1943"):
                try: supabase.table("essays").delete().eq("id", delete_options[sel]).execute(); st.success("Deleted!"); st.rerun()
                except Exception as e: st.error(f"Delete failed: {e}")
        else:
            st.info("No essays yet. Grade one to see here. If you just graded, wait 3 sec and press R to refresh.")
    except Exception as e: st.error(f"History error: {e}")

with tab1:
    essay = st.text_area("Paste Essay:", height=150, placeholder="Paste student essay..."); s_name = st.text_input("Student Name:", value="Student", key="s_name_single_1943"); level = st.selectbox("Target Level:", ["A1","A2","B1","B2","C1","C2"], key="single_level_1943")
    if st.button("🚀 GRADE & SAVE", type="primary", use_container_width=True):
        ...
        with st.spinner("Grading..."):
            try:
                ...
                if ok: st.success(f"✅ Saved: {score}/10 {cefr} | {ai_flag} | Check Portfolio tab (refresh if needed)")
                else: st.warning(f"Graded {score}/10 but save failed")
                st.markdown(result_text)
            except Exception as e: st.error(f"Grading error: {e}")

with tab3:
    st.markdown("### 📸 Photo & PDF")
    level_p=st.selectbox("Target Level:",["A1","A2","B1","B2","C1","C2"],key="photo_level_1943")
    camera_pic=st.camera_input("Take photo"); upload_img=st.file_uploader("Upload Image",type=["jpg","jpeg","png"],key="img_up_1943")
    image_bytes=None
    ...
    if image_bytes and st.button("📸 READ, GRADE & SAVE", type="primary", use_container_width=True, key="read_grade_save_1943"):
        with st.spinner("Reading + Grading..."):
            ...
            try:
                ...
                saved = save_essay_db(f"Photo {datetime.now().strftime('%H:%M:%S')}",extracted,level_p,score,cefr,result_text)
                if not is_pro(): st.session_state.uses+=1
                if saved:
                    st.success(f"✅ OCR + Graded + Saved: {score}/10 {cefr} | {ai_flag} | Go to Portfolio and press R to refresh")
                else:
                    st.error("Save failed - check login")
                st.text_area("OCR Text:", value=extracted, height=100, key="ocr_after_1943"); st.markdown(result_text)
            except Exception as e: st.error(f"Error: {e}")

with tab2:
    st.markdown("### ⚡ Batch 50 PRO")
    school_name = st.text_input("School Name:", value="My School", key="school_name_1943"); level_b = st.selectbox("Target Level:", ["A1","A2","B1","B2","C1","C2"], index=0, key="batch_level_1943")
    uploaded = st.file_uploader("Upload Excel/CSV (max 50)", type=["csv","xlsx","txt"], key="batch_file_1943")
    if st.button("🚀 GRADE BATCH 50", type="primary", use_container_width=True, key="batch_btn_50_1943"):
    ...
    if st.session_state.batch_results:
        ...
        if is_pro():
            st.download_button("📄 Principal PDF (PRO)", create_principal_pdf(excel_rows, level_b, avg_score, school_name), file_name=f"Principal_{school_name}.pdf", use_container_width=True, key="principal_pdf_1943")

st.caption("V1943 Deterministic - same essay = same score always")
