        ...
        try:
            res = client.chat.completions.create(
                ...
                messages=[{
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "You are OCR. Extract the handwritten or printed essay text EXACTLY as written, including spelling mistakes. Return only the essay text."},
...
with st.expander("📘 How to Use & Choose Target Level", expanded=True):
    st.markdown("1. **Paste** text OR **Photo/PDF**")
    st.markdown("2. **Pick Target Level** you WANT them to reach")
    st.markdown("3. **GRADE** = score + mistakes + corrected + PDF")
...
def grade_with_groq(essay_text, level):
    client = Groq(api_key=st.secrets["GROQ_API_KEY"])
    prompt = f"You are a Cambridge TEFL examiner for {level}. Grade this: {essay_text}. Return plain ASCII only, hyphen - only. Include CEFR Level, Score/10 vs Target {level}, Summary, 2 Strengths, Table Mistake|Correction|Why, Then corrected version."
    res = client.chat.completions.create(model="openai/gpt-oss-20b", messages=[{"role":"user","content":prompt}])
    return res.choices[0].message.content

tab1, tab2, tab3 = st.tabs(["Single Essay", "Batch 50 (PRO)", "📸 Photo / PDF"])

with tab1:
    essay = st.text_area("Paste Essay:", height=180)
...
with tab2:
    st.markdown("Upload CSV with column `essay` or TXT. PRO only.")
    level_b = st.selectbox("Target Level:", ["A1","A2","B1","B2","C1","C2"], key="batch_level")
    ...
    if st.button("GRADE BATCH 50 ->", key="batch_btn"):
        ...
        # FIXED FOR NEW FPDF2
        batch_pdf = pdf.output(dest='S').encode('latin-1')
...
with tab3:
    ...
    with colB:
        upload_pdf = st.file_uploader("Upload PDF", type=["pdf"], key="pdf_up")
