...
st.set_page_config(page_title="TEFLMate v6.1 SaaS", page_icon="📝", layout="centered")
...
def login_screen():
    ...
    with t2:
        email2 = st.text_input("New Email", key="signup_email")
        password2 = st.text_input("New Password", type="password", key="signup_pass")
        school = st.text_input("School Name", key="school_signup")
...
def create_principal_pdf(excel_rows, level, avg_score, school_name="School"):
    ...
    pdf.cell(0, 5, f"Generated by TEFLMate v6.1 SaaS | {school_name} | {datetime.now().strftime('%d %b %Y')}", align='C')
    return pdf.output(dest='S').encode('latin-1')
...
with col3:
    st.link_button(f"💳 Pay with PayPal", PAYPAL_ME)
st.caption("TEFLMate v6.1 SaaS • Durban, SA • Built by Mr Taahir Mahomed • Worldwide 🌍")
